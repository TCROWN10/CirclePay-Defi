...
var AgentServer = class {
  ...
  constructor() {
    try {
      ...
      logger30.error({ error }, "Failed to initialize AgentServer (constructor):");
      throw error;
    }
  }
  /**
  ...
  async initialize(options) {
    ...
    try {
      ...
      logger30.error({ error }, "Failed to initialize AgentServer (async operations):");
      console.trace(error);
      throw error;
    }
  }
  async ensureDefaultServer() {
    try {
      ...
      logger30.error({ error }, "[AgentServer] Error ensuring default server:");
      throw error;
    }
  }
  /**
  ...
  async initializeServer(options) {
    try {
      ...
      logger30.error({ error }, "Failed to complete server initialization:");
      throw error;
    }
  }
  /**
  ...
  async registerAgent(runtime) {
    try {
      ...
      logger30.error({ error }, "Failed to register agent:");
      throw error;
    }
  }
  /**
  ...
  unregisterAgent(agentId) {
    ...
    try {
      ...
      logger30.error({ error, agentId }, `Error removing agent ${agentId}:`);
    }
  }
  /**
...
\x1B[33mWeb UI disabled.\x1B[0m \x1B[32mAPI endpoints available at:\x1B[0m
  ...
  \x1B[1m${baseUrl}/api/messaging\x1B[22m\x1B[0m`
          ...
          logger30.error({ error, host, port }, `Failed to bind server to ${host}:${port}:`);
        ...
        logger30.error({ error }, "Failed to start server:");
        reject(error);
      }
    });
  }
    ...
    const gracefulShutdown = async () => {
      ...
      for (const [id, agent] of this.agents.entries()) {
        try {
          ...
          logger30.error({ error, agentId: id }, `Error stopping agent ${id}:`);
        }
      }
      if (this.database) {
        try {
          ...
          logger30.error({ error }, "Error closing database:");
        }
      }
      if (this.server) {
        this.server.close(() => {
